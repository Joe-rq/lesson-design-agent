...
本 Skill 负责 PPT 的**内容规划**（页数估算、内容筛选、大纲输出），实际 .pptx 文件生成使用 [PPT Master](https://github.com/hugohe3/ppt-master)。

### 工作流程

```
教案定稿 → 本 Skill 输出 PPT 内容大纲 → PPT Master 生成 .pptx
```

1. 阶段五（配套资源）会根据课时估算 PPT 页数（`课时 × 0.8 ~ 1.0`），并筛选保留内容
2. 输出每页标题 + 核心内容的 PPT 大纲
3. 将大纲提供给 PPT Master，自动生成可编辑的演示文稿
...
```
请根据 projects/ppt-outline.md 生成 PPT
```

PPT Master 支持：
- 原生可编辑的 DrawingML 元素（非图片导出），每个元素可直接在 PowerPoint 中点击编辑
- 多种风格（学术、商务、杂志、科技、自然纪录片等）
- 页面过渡动画和逐元素入场动画（原生 OOXML）
- 配音旁白 + 导出 MP4 视频
